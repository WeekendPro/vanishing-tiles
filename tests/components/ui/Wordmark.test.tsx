...
describe('Wordmark', () => {
  ...
  it('applies the lg size class when size="lg"', () => {
    ...
    expect(screen.getByRole('heading', { name: /phosphor/i }).className).toContain('text-3xl')
  })

  it('applies the sm size class by default', () => {
    render(<Wordmark />)
    expect(screen.getByRole('heading', { name: /phosphor/i }).className).toContain('text-lg')
  })

  it('merges a passed className', () => {
    render(<Wordmark className="mb-3" />)
    expect(screen.getByRole('heading', { name: /phosphor/i }).className).toContain('mb-3')
  })
})
