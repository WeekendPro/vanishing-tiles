...
src/
  ...
  store/
    ...
    navStore.ts            — appView routing state (auth/home/journey/levelDetail/playing/results/practice/stagger/training/leaderboard/soundDesign)
  ...
  lib/
    ...
    sfx.ts                 — Sound: semantic game-gesture API (pickCorrect/bloom/go/batchClear/…) + the ambient zen BED (startBed/stopBed, run-length meditation hum), all pure Web Audio synth (no audio files/assets); portable as-is to react-native-audio-api. Every sound is a PATCH (data: tone/noise layers) — DEFAULT_PATCHES/DEFAULT_BED are the shipped palette, overridable live via setPatch/setBedPatch (the Sound Design lab drives this). Two channels (SFX + music), each with toggle + volume in settingsStore; unlock() must fire from a tap (HomeScreen PLAY does)
  ...
  components/
    ...
    SoundDesignScreen.tsx  — The sound calibration lab (menu → "Sound Design", deliberately visible pre-launch): every game sound + the ambient bed as knob panels (per-layer pitch/length/loudness/attack/glide/lowpass, noise sweeps, bed voicing), ▶ replay with gameplay context (streak/reveal-step), labeled presets, Copy-JSON export
