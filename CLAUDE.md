...
src/
  ...
  lib/
    ...
    sfx.ts                 — Sound effects: semantic game-gesture API (pickCorrect/bloom/batchClear/…) over a pure Web Audio synth (no audio files/assets); portable as-is to react-native-audio-api. Mute rides settingsStore.soundEnabled; unlock() must fire from a tap (HomeScreen PLAY does)
